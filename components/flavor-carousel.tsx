...
const kelas = [
  {
    ...
    tagline: "GERAK SERU",
    description:
      "Kelas Zumba dasar dengan gerakan mudah diikuti. Cocok untuk pemula yang ingin mulai bergerak dan bakar kalori.",
  ...
  },
  {
    id: 2,
    name: "Zumba Fusion",
    tagline: "MAKIN NGEBUT",
    description:
      "Perpaduan Zumba dengan dance modern. Intensitas tinggi, keringat deras, dan pastinya seru abis!",
  ...
  },
  {
    id: 3,
    name: "Zumba Kids",
    tagline: "SEGER BANGET",
    description:
      "Kelas Zumba khusus untuk anak-anak. Gerakan fun dan energik bikin si kecil aktif dan ceria.",
...
export function FlavorCarousel() {
  ...
  const currentKelas = kelas[currentIndex];
  ...
  const paginate = (newDirection: number) => {
    const newIndex =
      (currentIndex + newDirection + kelas.length) % kelas.length;
    setCurrentIndex(newIndex);
    setPage([page + newDirection, newDirection]);
  };

  const nextKelas = () => paginate(1);
  const prevKelas = () => paginate(-1);

  return (
    <section id="kelas" className="relative py-16 bg-white overflow-hidden">
      <motion.div
        className={`absolute inset-0 bg-gradient-to-br ${currentKelas.bgColor}`}
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6">
        ...
        >
          ...
          <h2 className="text-3xl md:text-5xl font-black text-[#121212] tracking-tighter mt-2 overflow-hidden">
            ...
            <motion.span
              className="inline-block"
              style={{ color: currentKelas.accentColor }}
              initial={{ y: 80 }}
              whileInView={{ y: 0 }}
              viewport={{ once: true }}
              transition={{
        ...
        <div className="relative">
          <div className="flex items-center justify-center gap-6">
            ...
            <AnimatePresence mode="wait" custom={direction}>
              ...
              >
                <motion.div
                  className={`bg-white rounded-3xl p-6 md:p-8 border-2 border-[#121212]/10 shadow-xl ${currentKelas.mystery ? "relative overflow-hidden" : ""}`}
                  style={{ rotateX, rotateY, transformStyle: "preserve-3d" }}
                  onMouseMove={handleMouseMove}
                  onMouseLeave={handleMouseLeave}
                >
                  {currentKelas.mystery && (
                    <motion.div
                  ...
                  <div className="grid md:grid-cols-2 gap-6 items-center">
                    ...
                    >
                      <Image
                        src={currentKelas.image || "/placeholder.svg"}
                        alt={currentKelas.name}
                        fill
                        className={`object-contain ${currentKelas.mystery ? "blur-sm grayscale" : ""}`}
                      />
                      {currentKelas.mystery && (
                        <motion.div
                    ...
                    <div className="space-y-4">
                      ...
                      {!currentKelas.mystery && (
                        <motion.button
                          ...
                          style={{
                            backgroundColor: currentKelas.accentColor,
                            color: "#121212",
                          }}
                          initial={{ opacity: 0, y: 20 }}
                          animate={{ opacity: 1, y: 0 }}
                        >
                          ...
                          />
                          <span className="relative z-10">Daftar Kelas</span>
                        </motion.button>
                      )}
          ...
          <div className="flex justify-center gap-2 mt-6">
            {kelas.map((kls, index) => (
              <motion.button
                ...
                style={{
                  backgroundColor:
                    index === currentIndex ? kls.accentColor : "#12121220",
                }}
                animate={{
